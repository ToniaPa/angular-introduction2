...
export class JokesService {
  ...
  //η http που είναι HttpClient επιστρέφει observables που απ'ότι κατάλαβα είναι ένας πινακας από observables ο οποίος είναι δεμένος με το http που φτιάξαμε με τον παραπάνω τρόπο => όλα όσα επιστρέφουν οι servers που χτυπάμε με το http που φτιάξαμε (κάνοντας get, post κλπ σε link) ειναι και ένα observable που μπαίνει στο array των observables του http και πλέον παρατηρεί για την απάντηση από τον server που χτυπήσαμε

  getDadJoke() {
    return this.http.get<DadJoke>(DAD_JOKES_API_URL, { //<DadJoke> = interface στο '../interfaces/jokes'
      headers: {
        Accept: 'application/json', //= Observable, επιστέφει ένα Observable που μπαίνει στο array των observables (που έχει δεθεί με το http μας)
      },
    });
  }

  getChuckNorrisJoke() { 
    return this.http.get<ChuckNorrisJoke>(CHUCK_NORRIS_JOKES_API_URL, { //<ChuckNorrisJoke> = interface στο '../interfaces/jokes'
      headers: {
        Accept: 'application/json', //= Observable, επιστέφει ένα Observable που μπαίνει στο array των observables (που έχει δεθεί με το http μας)
      },
    });
